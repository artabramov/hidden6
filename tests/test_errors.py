...
class TestS3ErrorSubclasses(unittest.TestCase):
    CASES = (
        ...
        (
            S3ObjectKeyInvalidError,
            S3ErrorCode.OBJECT_KEY_INVALID,
            status.HTTP_400_BAD_REQUEST,
        ),
        (
            S3ObjectKeyConflictError,
            S3ErrorCode.OBJECT_KEY_CONFLICT,
            status.HTTP_400_BAD_REQUEST,
        ),
        (
            S3ObjectTooLargeError,
        ...
        (
            S3ObjectBodyIncompleteError,
            S3ErrorCode.OBJECT_BODY_INCOMPLETE,
            status.HTTP_400_BAD_REQUEST,
        ),
        (
            S3ObjectUploadNotFoundError,
            S3ErrorCode.OBJECT_UPLOAD_NOT_FOUND,
            status.HTTP_404_NOT_FOUND,
        ),
        (
            S3ObjectPartNumberInvalidError,
            S3ErrorCode.OBJECT_PART_NUMBER_INVALID,
            status.HTTP_400_BAD_REQUEST,
        ),
        (
            S3ObjectPartInvalidError,
            S3ErrorCode.OBJECT_PART_INVALID,
            status.HTTP_400_BAD_REQUEST,
        ),
        (
            S3ObjectPartOrderInvalidError,
            S3ErrorCode.OBJECT_PART_ORDER_INVALID,
            status.HTTP_400_BAD_REQUEST,
        ),
        (
            S3ObjectPartTooSmallError,
            S3ErrorCode.OBJECT_PART_TOO_SMALL,
            status.HTTP_400_BAD_REQUEST,
        ),
        (
            S3XmlMalformedError,
