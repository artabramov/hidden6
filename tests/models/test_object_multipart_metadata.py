# tests/models/test_object_multipart_metadata.py
...
from app.models.object_multipart_metadata import S3ObjectMultipartMetadata  # noqa: E402
...
class TestS3ObjectMultipartMetadataModel(unittest.TestCase):
    ...
    def _metadata(self, **kwargs) -> S3ObjectMultipartMetadata:
        defaults = {
        ...
        return S3ObjectMultipartMetadata(**defaults)

    def test_tablename(self):
        self.assertEqual(
            S3ObjectMultipartMetadata.__tablename__,
    ...
    def test_same_meta_key_allowed_on_different_multiparts(self):
        ...
        rows = self.session.scalars(select(S3ObjectMultipartMetadata)).all()
    ...
    def test_relationship_back_to_multipart(self):
        ...
        loaded = self.session.scalar(
            select(S3ObjectMultipartMetadata)
            .where(S3ObjectMultipartMetadata.id == row.id)
            .options(
                selectinload(
                    S3ObjectMultipartMetadata.object_multipart_metadata_object_multipart,
                ),
            ),
        )
    ...
    def test_cascade_delete_with_multipart(self):
        ...
        remaining = self.session.scalars(select(S3ObjectMultipartMetadata)).all()
