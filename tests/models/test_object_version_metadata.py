...
class TestS3ObjectVersionMetadataModel(unittest.TestCase):
    ...
    def _metadata(self, **kwargs) -> S3ObjectVersionMetadata:
        defaults = {
        ...
        return S3ObjectVersionMetadata(**defaults)

    def test_tablename(self):
        self.assertEqual(
            S3ObjectVersionMetadata.__tablename__,
    ...
    def test_same_meta_key_allowed_on_different_versions(self):
        ...
        rows = self.session.scalars(select(S3ObjectVersionMetadata)).all()
    ...
    def test_relationship_back_to_version(self):
        ...
        loaded = self.session.scalar(
            select(S3ObjectVersionMetadata)
            .where(S3ObjectVersionMetadata.id == row.id)
            .options(
                selectinload(
                    S3ObjectVersionMetadata.object_version_metadata_object_version,
                ),
            ),
        )
    ...
    def test_cascade_delete_with_version(self):
        ...
        remaining = self.session.scalars(select(S3ObjectVersionMetadata)).all()
