# tests/services/test_bucket_objekt_lock_update.py
...
from app.services.bucket_objekt_lock_update import (  # noqa: E402
...
class TestBucketObjektLockUpdate(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.log_patcher = patch("app.services.bucket_objekt_lock_update.log")
    ...
    async def _update(self, bucket, body, repo=None, user=None):
        ...
        with (
            patch(
                "app.services.bucket_objekt_lock_update.ORMRepository",
                return_value=repo,
            ),
            patch(
                "app.services.bucket_objekt_lock_update.load_bucket",
                new_callable=AsyncMock,
                return_value=bucket,
            ) as load_bucket_mock,
            patch(
                "app.services.bucket_objekt_lock_update.hooks.emit",
                new=emit,
            ),
        ):
